...
def phrases():
    ...
    P.update(['かきじゅんで かいてみよう', 'ここから かいてね', 'もういちど', 'じょうず！'])   # かきじゅんモード
    # しりとり
    ...
    # ---- 追加ゲーム（18種・2026-09-22 第3弾）----
    # おかいもの
    P.update(['コインを おいて はらおう', 'ぴったり！ ありがとう！', 'まだ たりないよ', 'おおすぎるよ'])
    P.update(['ひゃくえん', 'にひゃくえん', 'さんびゃくえん', 'よんひゃくえん', 'ごひゃくえん', 'ろっぴゃくえん', 'ななひゃくえん', 'はっぴゃくえん'])
    # なんばんめ
    NTH = ['いちばんめ', 'にばんめ', 'さんばんめ', 'よんばんめ', 'ごばんめ', 'ろくばんめ']
    P.update(['まえから', 'うしろから'] + [f'{n}は どれ？' for n in NTH] + [f'{n}！' for n in NTH])
    # はんぶんこ
    KO = ['いっこ', 'にこ', 'さんこ', 'よんこ', 'ごこ', 'ろっこ', 'ななこ', 'はっこ']
    P.update(['おなじ かずに わけよう', 'おなじ かず！ はんぶんこ できた！', 'かずが ちがうよ。 もういちど'] + [f'{k}と' for k in KO] + [f'{k}！' for k in KO] + [f'{k}。' for k in KO])
    # おはなしのじゅんばん
    P.update(['じゅんばんに ならべよう', 'じゅんばんが ちがうよ。 もういちど',
              'たねを まいて、 めが でて、 はなが さいた！', 'たまごが われて、 ひよこが うまれて、 にわとりに なった！',
              'あおむしが さなぎに なって、 ちょうちょに なった！', 'こねて、 やいて、 パンが できた！',
              'おふろに はいって、 パジャマを きて、 ねんね！', 'くもが きて、 あめが ふって、 にじが でた！'])
    # ぶんるい
    P.update(['おおきいのは どれ？', 'ちいさいのは どれ？', 'いろと おおきさを よくみてね'])
    # かげのむき
    P.update(['おなじ むきの かげは どれ？', 'むきが ちがうよ', 'おなじ むき！'])
    # どうさのことば
    VERBS = ['はしる', 'たべる', 'ねる', 'のむ', 'あるく', 'とぶ', 'なく', 'わらう', 'よむ', 'かく', 'うたう', 'おどる', 'みつける', 'もらう', 'なげる', 'あらう']
    for vb in VERBS:
        P.update([f'{vb}は どれかな？', f'{vb}！', f'それは {vb}。'])
    # なきごえあて
    CRY = [('いぬ', 'わんわん'), ('ねこ', 'にゃーにゃー'), ('ぞう', 'ぱおーん'), ('らいおん', 'がおー'), ('うし', 'もーもー'), ('かえる', 'けろけろ'), ('さる', 'うきうき'), ('ぶた', 'ぶーぶー'),
           ('にわとり', 'こけこっこー'), ('ひよこ', 'ぴよぴよ'), ('ひつじ', 'めーめー'), ('うま', 'ひひーん'), ('ねずみ', 'ちゅーちゅー'), ('あひる', 'がーがー')]
    P.update(['なんの なきごえ かな？'])
    for n, c in CRY:
        P.update([n, c, f'{n}の なきごえは どれ？', f'{n}だね'])
    # ものがたりをつくる
    P.update(['だれが なにを どうした？ えらんでね', 'たべた！', 'みつけた！', 'もらった！', 'なげた！', 'あらった！', 'かいた！'])
    # リズムたたき
    P.update(['よく きいてね', 'きみの ばん！ おなじ かず たたいて！', 'おなじ かず！ じょうず！', 'かずが ちがったよ。 もういちど'])
    # かぞえてジャンプ
    KAI = ['いっかい', 'にかい', 'さんかい', 'よんかい', 'ごかい', 'ろっかい', 'ななかい', 'はっかい', 'きゅうかい', 'じゅっかい']
    P.update([f'{k} タッチ！' for k in KAI])
    P.update(['それから ジャンプの ボタンを おしてね', 'ジャンプ！', 'とべた！ やったね', 'おおすぎたよ。 もういちど', 'たりないよ。 もういちど'])
    # おべんとう
    P.update(['すきな おかずを いれよう', 'もう いっぱいだよ', 'おかずを いれてね', 'おいしそうな おべんとう！ できた！',
              'たまごやき', 'ウインナー', 'ブロッコリー', 'ミニトマト', 'からあげ', 'ぶどう'])
    # おうち
    P.update(['パーツを えらんで おうちを つくろう', 'やねと ドアを つけてね', 'すてきな おうち！ できた！',
              'かべ', 'あかい やね', 'あおい やね', 'みどりの やね', 'ちゃいろの ドア', 'あかい ドア', 'しかくい まど', 'まるい まど', 'えんとつ', 'き', 'おはな', 'おひさま', 'くも'])
    # ロボット
    P.update(['パーツを えらんで ロボットを つくろう', 'ぜんぶの パーツを つけてね', 'ロボット、 かんせい！ うごくよ！', 'あたま', 'からだ', 'うで', 'あし'])
    # はみがき
    P.update(['はぶらしを うごかして ばいきんを やっつけよう', 'ぴかぴか！ きれいに なった！'])
    # おきがえ
    P.update(['つぎは なにを きる？', 'じゅんばんが ちがうよ。 つぎは なにかな？', 'おきがえ できた！ いってきます！',
              'パンツ！', 'ズボン！', 'シャツ！', 'くつした！', 'くつ！', 'ぼうし！'])
    # おてつだい
    P.update(['どこに もっていく？ うごかしてね', 'ぜんぶ おてつだい できた！ ありがとう！', 'そこじゃないよ',
              'ながしに かたづけた！', 'ゴミばこに かたづけた！', 'おもちゃばこに かたづけた！',
              'おさら', 'コップ', 'おわん', 'スプーン', 'バナナの かわ', 'おかしの ふくろ', 'かみくず', 'ボール', 'つみき', 'くまさん'])
    return sorted(P)
