...
| `blocks.html` | つみき | しかく・ながしかく・さんかく・まる を棚からつまんで落とし、台の上に積む（物理は つみあげタワー の流用 `blocks_sim.js`）。落ちたら消える。お題はなく無制限に積める。1 こ乗るたびに ⭐、5 こでシール（積めた数がそのままレベル）。「つめた N こ／さいこう M こ」を表示、🔄 長押しで台を空に |
