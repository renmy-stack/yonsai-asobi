...
メニューは「かず / あたま / ことば / うごき / つくる / せいかつ」の6つのタブに分かれる（1画面に最大15個）。

| ページ | 名前 | 内容 |
|--------|------|------|
| `quiz.html` | どこかな？ | 6枚の中から「〇〇はどこ？」と言われたものをタッチ。5問目からは全部まぜる |
| `add.html` | たしざん | 2＋1 などを絵で数えて答える。助数詞つき（「いぬが にひきと いっぴき。あわせて なんびき？」） |
| `sub.html` | ひきざん | 「りんごが さんこ。いっこ たべた。のこりは なんこ？」右はしから消えて（×で残る）、残りを数えて答える。助数詞・いなくなり方はものに合わせる（どうぶつ: いなくなった／のりもの: いっちゃった／たべもの: たべた）。2−1 → 6−5、レベル4から4択 |
...
| `trace.html` | なぞりがき | かたち・すうじ・ひらがなを指でなぞる（7割塗れたら合格）。「かきじゅん」モードでは1画ずつお手本アニメ→なぞって書き順の練習（始点・向き・終点を判定。2回続けて失敗するとゆるくなる） |
...
| `seal.html` | シールあそび | 集めたシールを背景（おへや・こうえん・うみ・うちゅう）に貼って絵を作る。タッチで貼る→指で動かす、おおきく・ちいさく・むき・はがす。📷 で1枚の絵にして保存（長押し）。貼れるのは持っている枚数まで（棚に残り枚数、はがすと戻る） |
| `draw.html` | おえかき | 自由に描く。9色・太さ・けしごむ・キャラのスタンプ |
| `piano.html` | ピアノ | 8鍵。きらきらぼし等4曲を自動演奏して鍵盤が光る |
| `shop.html` | おかいもの | 「りんごは さんびゃくえん」100円コインを置いて「はらう」。多い・少ないも教える。100〜300円 → 100〜800円 |
| `nth.html` | なんばんめ？ | 並んだ動物の「まえから 2ばんめ」をタッチ。レベル2から「うしろから」も |
| `share.html` | はんぶんこ | おやつを2つのお皿に同じ数ずつ分ける（2〜8個） |
| `story.html` | おはなし | 3コマの絵（たね→め→はな 等6話）を正しい順に並べる |
| `classify.html` | ぶんるい | 「あかの おおきいのは どれ？」色と大きさの2条件で選ぶ。4択 → 6択 |
| `rotate.html` | かげのむき | 回転した絵と同じ向きの影を選ぶ。2択 → 4択 |
| `karuta.html` | かるた | 読み上げを聞いて絵札をタッチ。4枚 → 8枚 |
| `verb.html` | どうさのことば | 「たべる は どれ？」動詞16語を絵で |
| `cry.html` | なきごえあて | 動物の鳴き声（わんわん 等14種）を文字で選ぶ。レベル2から「声を聞いて動物を当てる」も |
| `sentence.html` | ものがたり | だれが・なにを・どうした を選ぶと文になって読み上げる |
| `rhythm.html` | リズムたたき | 太鼓の音を聞いて同じ回数たたく（1〜7回） |
| `roll.html` | ボールころがし | 指で線を描いて（固い木の線、何本でも）、棚のボールをカゴまで転がす。↩ で最後の線を消す・🗑 で全部消す・「ころがす！」。落ちても線は残る。レベルが上がるとカゴが遠くなり、岩が出る |
| `jump.html` | かぞえてジャンプ | 数字の回数タッチして岩を飛び越える。レベル2からは自分で「ジャンプ」を押す |
| `bento.html` | おべんとう | おかず8種をお弁当箱に詰める |
| `house.html` | おうちづくり | やね・ドア・まど・き・おひさま などを選んで家を作る |
| `blocks.html` | つみき | しかく・ながしかく・さんかく・まる を棚からつまんで落とし、台の上に積む（物理は つみあげタワー の流用 `blocks_sim.js`）。落ちたら消える。お題・レベル・シールはなく無制限に積む自由あそび（おえかき と同じ扱い）。「つめた N こ／さいこう M こ」を表示（5 こごとに「すごい！」）、🔄 長押しで台を空に |
| `robot.html` | ロボット | あたま・からだ・うで・あし を3種から選んで組み立て、動かす |
| `teeth.html` | はみがき | 歯ブラシで歯のばいきんをこすって落とす（3〜7匹） |
| `dress.html` | おきがえ | パンツ→ズボン→シャツ→くつした→くつ→ぼうし の順に選ぶ |
| `help.html` | おてつだい | 食器はながし、ゴミはゴミばこ、おもちゃはおもちゃばこへドラッグ |

## ごほうび・親向け

- **シール帳** (`stickers.html`): どのゲームでも5問正解するとシールが1枚もらえる（26種。トロフィーと犬は出にくい「きらきらシール」）。全種類集めると特別なお祝い。メニュー右上から開く
- **おうちのひと** (`parent.html`): メニュー左上の ⚙️ を2秒長押し → 「4＋6＝？」の計算に答えると開く。おしまいタイマー（時間が来ると「きょうは おしまい」画面。30分放置で自動解除）、遊んだ記録（回数・時間・正解率・最後に遊んだ日、きょうの合計）、メニューに出すゲームの選択、タイトル変更、ぜんぶダウンロード（オフライン用）、データ消去（きろく／レベル／シール／ぜんぶ を別々に。設定は残る）、クレジット
- **レベル**: 5問正解ごとに1つ上がる（最大5、メダル 🥉🥈🥇👑💎）。難しさはレベル1〜5で段階的に変わる（個数・選択肢の数・覚える時間・速さ・紛らわしさ。例: たしざん 1+1 → 5+1・4択、とけい 1〜6じ → 12じ → はん → 近い時刻の4択、どこかな？ 4枚 → 6枚 → まぜる → 8枚）。メニューのタイルとゲーム画面上部に表示し、上がった瞬間に「レベルアップ！」の演出。親ページで1つ下げられる
...
- `strokes.js` — なぞりがき「かきじゅん」用の書き順データ（ひらがな46・すうじ・かたち。ひらがなは KanjiVG）
- `assets/verbs|story|bento|house|robot|life/` — 追加ゲーム用の絵（ChatGPT 生成、シートを分割）
...
- `strokes.js` に 1画 = SVG パス1本で入っている（座標系 109×109）
- ひらがな46文字は KanjiVG（https://kanjivg.tagaini.net/ 、Ulrich Apel、CC BY-SA 3.0）の筆順データ。クレジット: 「KanjiVG (CC BY-SA 3.0)」
- かたち・すうじは自作。すうじは日本の小学校でよく教える書き方（1は縦棒1本、4・5は2画、7は1画）
